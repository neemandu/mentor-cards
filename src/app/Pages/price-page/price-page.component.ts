...
export class PricePageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log(
      'file: price-page.component.ts ~ line 80 ~ ngOnInit ~ this.userData',
      this.userData
    );
  }
  ...
  getSubscriptionPlans(): void {
    ...
    this.halfYearlySubscriptionPercentage = Math.round(
      100 -
        (this.halfYearlySubscription?.fullPrice * 100) /
          (this.monthlySubscription?.fullPrice *
            this.halfYearlySubscription?.billingCycleInMonths)
    );
    this.yearlySubscriptionPercentage = Math.round(
      100 -
        (this.yearlySubscription?.fullPrice * 100) /
          (this.monthlySubscription?.fullPrice *
            this.yearlySubscription?.billingCycleInMonths)
    );
  ...
  get paymentStartDate() {
    if (this.userData) {
      return new Date(
        this.userData.firstProgramRegistrationDate.getTime() +
          millisecondsInTwoWeeks
      );
    }
  ...
  checkFreePeriod(packId): void {
    if (!this.userSingedIn) {
      ...
      if (this.userData?.status === 'PLAN') {
        ...
        const content = [
          'שימו לב!',
          `התכנית החדשה תעודכן החל מהתאריך ${this.nextPaymentDate}`,
        ];
  ...
  private get nextPaymentDate() {
    let cycles =
      this.userData.subscription.subscriptionPlan?.billingCycleInMonths;
    let now = new Date();
    ...
    return new Date(
      createdAt.getTime() + numberOfMonthsToAdd
  ...
  contactUsForTeams(): void {
    this.mixpanelService.track('ButtonClicked', {
      Name: 'Contact us for teams',
    });
    // You can implement navigation to contact page or open a contact modal
  ...
  onBillingToggleChange(): void {
    this.mixpanelService.track('ActionButtonClicked', {
      ...
      'Billing Type': this.isAnnualBilling ? 'Annual' : 'Monthly',
    });
  }
