...
export class ApprovePurchaseDialogComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<ApprovePurchaseDialogComponent>,
    ...
    paypal
      .Buttons({
        createSubscription: (data, actions) => {
          //lastPlanSubstitutionDate - once in last 30 days
          // debugger
          if (
            this.userAuthService.userData?.status === 'NOPLAN' ||
            this.data.packId
          )
            ...
            return actions.subscription.revise(
              this.userAuthService.userData.subscription.providerTransactionId,
              {
                plan_id: plan_id,
                custom_id: this.userAuthService.userData?.id,
              }
        ...
        onApprove: async (data, actions) => {
          this.mixpanel.track('PlanPurchase', {
            Provider: 'PayPal',
            'Plan ID': plan_id,
            ...
            'Billing cycle':
              this.data.subscriptionPlanSelected?.billingCycleInMonths,
          ...
          var ids: updatePaymentProgramInput = {
            paymentProgramId: this.data.subscriptionPlanSelected.id,
            providerTransactionId: data.subscriptionID,
            packId: this.data.packId ? this.data.packId : -1,
          };
          this.api.UpdatePaymentProgram(ids).then(
            (data) => {
              this.userAuthService._snackBar.open(
                'הרשמתך לתכנית בוצעה בהצלחה!',
                '',
                {
                  duration: 4000,
              ...
              this.sharedDialogsService.openFacebookWelcomeDialog(
                this.data.subscriptionPlanSelected
              );
            },
            (error) => {
              this.overlaySpinnerService.changeOverlaySpinner(false);
              console.log(
                '🚀 ~ file: program-choise-dialog.component.ts ~ line 71 ~ ProgramChoiseDialogComponent ~ this.api.UpdatePaymentProgram ~ error',
                error
              );
            }
          );
        ...
        onError: (err) => {
          console.log(
            '🚀 ~ file: program-choise-dialog.component.ts ~ line 77 ~ ProgramChoiseDialogComponent ~ stepChanged ~ err',
            err
          );
        },
        style: {
